...
public class CHeaderConnector implements IFrancaConnector {
	...
	public IModelContainer loadModel(String fileName) {
		...
		ITranslationUnit translationUnit = CoreModelUtil.findTranslationUnit(workspaceRoot.getFileForLocation(location));
		if (translationUnit == null) {
			return null;
		}
		else {
			return new CHeaderModelContainer(translationUnit, fileName);
		}
	}
	...
	public FModel toFranca(IModelContainer model) {
		...
		return transformation.transform(modelContainer.getTranslationUnit(), modelContainer.getFileName());
	}

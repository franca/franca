...
public class FrancaIDLJavaValidator extends AbstractFrancaIDLJavaValidator
	...
	public void checkTypeNamesUnique(FModel model) {
		ValidationHelpers.checkDuplicates(this, model.getTypes(),
	...
	public void checkCompoundElementsUnique (FCompoundType type) {
		ValidationHelpers.checkDuplicates(this, type.getElements(),
				FrancaPackage.Literals.FCOMPOUND_TYPE__ELEMENTS, "element name");
	}
	...
	public void checkTypeDefSelfRef (FTypeDef type) {
		if (type.getActualType().getDerived()!=null) {
			if (type==type.getActualType().getDerived()) {
				error("Cyclic inheritance for FTypeDef",
						type, FrancaPackage.Literals.FTYPE_DEF__ACTUAL_TYPE, -1);
			}
		}
	}
	...
	public void checkMapSelfRef (FMapType type) {
		...
		if (keyType!=null) {
			if (type==keyType) {
				error("Map references itself",
						type, FrancaPackage.Literals.FMAP_TYPE__KEY_TYPE, -1);
			}
		}
		if (valueType!=null) {
			if (type==valueType) {
				error("Map references itself",
						type, FrancaPackage.Literals.FMAP_TYPE__VALUE_TYPE, -1);
			}
		}
	}
	...
	public void checkStructSelfExtend (FStructType type) {
		if (type.getBase()!=null) {
			if (type==type.getBase()) {
				error("Cyclic inheritance for struct",
						type, FrancaPackage.Literals.FSTRUCT_TYPE__BASE, -1);
			}
		}
	}
	...
	public void checkUnionSelfExtend (FUnionType type) {
		if (type.getBase()!=null) {
			if (type==type.getBase()) {
				error("Cyclic inheritance for union",
						type, FrancaPackage.Literals.FUNION_TYPE__BASE, -1);
			}
		}
	}
	...
	public void checkEnumSelfExtend (FEnumerationType type) {
		if (type.getBase()!=null) {
			if (type==type.getBase()) {
				error("Cyclic inheritance for FEnumerationType",
						type, FrancaPackage.Literals.FENUMERATION_TYPE__BASE, -1);
			}
		}
	}
			...
			if (refParent==null) {
				...
				if (refParent!=parent) {
					error("Type " + referencedType.getName() +
							" can only be referenced inside interface " + refParent.getName(),
							typeref, FrancaPackage.Literals.FTYPE_REF__DERIVED, -1);
				}
			}
		}
	}

...
class FrancaIDLValidator {
	...
	def checkCompoundElementsUnique (FCompoundType type) {
		...
		checkDuplicates(reporter, elements,
				FMODEL_ELEMENT__NAME, "element name")
		...
		for(f : elements) {
			...
			if (baseNames.containsKey(n)) {
				reporter.reportError(
					"Element name collision with base element '" + baseNames.get(n) + "'",
					f, FMODEL_ELEMENT__NAME)
			}
		}
	}
	...
	def checkUnionElementTypesUnique (FUnionType type) {
		...
		for(f : type.elements) {
			names.add(f, getName(f.type))
		}
		checkDuplicates(reporter, names,
				FTYPED_ELEMENT__TYPE, "element type")

		val baseNames = getBaseElementNames(type, [it.type])
		for(f : type.elements) {
			val n = f.type.name
			if (baseNames.containsKey(n)) {
				reporter.reportError("Element type collision with base element '" + baseNames.get(n) + "'",
						f, FTYPED_ELEMENT__TYPE);
			}
		}
	}
	...
	def checkEnumeratorsUnique (FEnumerationType type) {
		...
		checkDuplicates(reporter, elements,
				FMODEL_ELEMENT__NAME, "enumerator name");
		...
		for(f : elements) {
			...
			if (baseNames.containsKey(n)) {
				reporter.reportError(
					"Enumerator name collision with base element '" + baseNames.get(n) + "'",
					f, FMODEL_ELEMENT__NAME)
			}
		}
	}

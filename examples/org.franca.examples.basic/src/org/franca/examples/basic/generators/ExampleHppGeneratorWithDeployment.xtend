...
import org.franca.examples.deployspecs.SampleDeploySpecInterfacePropertyAccessor
import org.franca.examples.deployspecs.SampleDeploySpecInterfacePropertyAccessor$CallSemantics
import org.franca.examples.deployspecs.SampleDeploySpecInterfacePropertyAccessor$Encoding
...
class ExampleHppGeneratorWithDeployment {
	...
	def generateInterface (FInterface api) '''
		...
		private:
			// attributes
			«FOR a : api.attributes»
			«a.type.generate» m«a.name.toFirstUpper»;«IF deploy.getIsReadOnly(a)» // read only«ENDIF»
			«ENDFOR»
		};
